...
public class TwoThreeTreeTest {
    public static void main(String[] args) {
        ...
        String[][] elements = new String[rows.length][7];
        for(int i = 0; i < rows.length; i++){
            elements[i] = rows[i].split(", ");
            twoThreeTree.insert(Integer.parseInt(elements[i][0]),elements[i][1],Float.parseFloat(elements[i][2]),Integer.parseInt(elements[i][3]),Float.parseFloat(elements[i][4]),Integer.parseInt(elements[i][5]),Integer.parseInt(elements[i][6].substring(0,elements[i][6].length()-1)));
        }
        twoThreeTree.treeDetour();

    }
    private static String readFile(){
        String file = "";
        try{
            FileInputStream fin = new FileInputStream("C:\\Users\\manke\\IdeaProjects\\Lab1\\src\\prac31\\PROCS.txt");
            int i = -1;
            while((i = fin.read()) != -1){
                file += Character.toString((char)i);
            }
...
class TwoThreeTree {
    ...
    private class LeafNode extends Node {
        ...
        LeafNode(int key, String name, float freq, int cashMemorySize, float busFreq, int SPECint, int SPECfp) {
            this.key = key;
            this.name = name;
            this.freq = freq;
            this.cashMemorySize = cashMemorySize;
    ...
    private void insertKey(int key,String name,float freq,int cashMemorySize, float busFreq, int SPECint , int SPECfp) {
        // Create an array of Node of size 2
        // to capture the 2 nodes
        Node[] array = new Node[2];
        // Call the method to insert the key
        array = insert(key,name,freq,cashMemorySize,busFreq,SPECint,SPECfp, root);
    ...
    private Node[] insert(int key,String name,float freq,int cashMemorySize, float busFreq, int SPECint , int SPECfp, Node n) {
        ...
        if (root == null && !first) {
            ...
            t.children[0] = insert(key,name,freq,cashMemorySize,busFreq,SPECint,SPECfp, t.children[0])[0];
        ...
        // stores references to TreeNodes
            ...
            if (key < t.keys[0]) {
                ...
                catchArray = insert(key,name,freq,cashMemorySize,busFreq,SPECint,SPECfp, t.children[0]);
            ...
            // or second is null
                ...
                catchArray = insert(key,name,freq,cashMemorySize,busFreq,SPECint,SPECfp, t.children[1]);
                // Store the reference returned by the call
            ...
            // If the key to be inserted is greater than second key
                ...
                catchArray = insert(key,name,freq,cashMemorySize,busFreq,SPECint,SPECfp, t.children[2]);
                // Store the reference returned by the call
        ...
        // stores references to LeafNodes
            ...
            if (t.degree <= 2) {
                ...
                if (t.degree == 1 && key > l1.key) {
                    LeafNode leaf = new LeafNode(key,name,freq,cashMemorySize,busFreq,SPECint,SPECfp);
                    t.children[1] = leaf;
            ...
            successfulInsertion = true;
            array[0] = new LeafNode(key,name,freq,cashMemorySize,busFreq,SPECint,SPECfp);
            array[1] = null;
            return array;
        }
    ...
    public boolean insert(int key,String name,float freq,int cashMemorySize, float busFreq, int SPECint , int SPECfp) {
        // Let insert to false
        ...
        if (!search(key)) {
            ...
            insertKey(key,name,freq,cashMemorySize,busFreq,SPECint,SPECfp);
        }
    ...
    public void treeDetour(){
        ...
        for(int i = layers.size()-1; i >= 0; i--){
            for(int j = 0; j < layers.get(i).size(); j++){
                if(layers.get(i).get(j) instanceof LeafNode){
                    LeafNode treeNode = (LeafNode) layers.get(i).get(j);
                    treeNode.print();
                } else{
                    TreeNode leafNode = (TreeNode) layers.get(i).get(j);
                    leafNode.print();
                }
            }
            System.out.println("");
